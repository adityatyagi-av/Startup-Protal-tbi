      ...
      <table className="w-full bg-white border-collapse shadow-sm">
        <thead>
          <tr className="text-blue-900 bg-blue-100">
            ...
            <th className="p-4 text-left">Requested Item</th>
        ...
        <tbody>
          {items && items.length > 0 ? (
            items.map(({ name, createdAt, status }, index) => (
              <tr key={index} className="border-b hover:bg-gray-100">
                <td className="p-4">{index + 1}</td>
                <td className="p-4">{name || "N/A"}</td>
                <td className="p-4">{createdAt ? new Date(createdAt).toLocaleDateString() : "N/A"}</td>
