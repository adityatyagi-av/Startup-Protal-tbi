...
import { GET_CHAT_USER  ,GET_CHAT_MESSAGES, SEARCH_CHAT_USER} from "../../../APiEndPoints/ApiEndPoints";
...
export const FetchChatUser = () => async (dispatch) => {
  try {
    ...
    if (response?.data?.success) {
      ...
      toast.error(response.data.message || "Failed to fetch evaluation questions");
    }
  } catch (error) {
    console.log(error);
    dispatch({
...
export const fetchMessages = (conversationId, itemsPerPage = 10, currentPage = 1) => async (dispatch) => {
...
export const deleteMessages = (messageIds ) => async (dispatch) => {
  ...
  try {
    const  response = await axios.put(`${process.env.NEXT_PUBLIC_DATABASE_URL}/chat/deleteMessages`, 
